  ...
  <div class="histories-container">
    ...
    <el-table :data="histories" border stripe v-loading="loading" :row-key="getRowKey" empty-text="暂无历史记录">
      ...
      <el-table-column label="单号" min-width="140" show-overflow-tooltip>
        <template #default="{ row }">
          <el-link type="primary" @click.prevent="openRecord(row.result.ticket_link)">
            {{ row.result.display_value }}
          </el-link>
        </template>
      </el-table-column>
...
<script setup lang="ts">
  ...
  const {
    state: histories,
    isLoading: loading,
    execute: executeLoadHistories,
  } = useAsyncState(() => window.electron.getTicketHistory(), [] as TicketHistoryItem[], {
    immediate: true,
    resetOnExecute: false,
  })
  const { execute: executeClearHistories } = useAsyncState(() => window.electron.clearTicketHistory(), undefined, {
    immediate: false,
    resetOnExecute: false,
  })
  ...
  const copyTicket = async (history: TicketHistoryItem) => {
    ...
    ticketStore.setHistoryCopyPayload({
      ...
      requestFor: history.ticket.requestFor ?? '',
    ...
    await router.push({
      ...
      query: {
        ...
        copyRequestFor: history.ticket.requestFor ?? '',
