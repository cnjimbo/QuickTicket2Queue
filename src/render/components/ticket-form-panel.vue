<script setup lang="ts">
  ...
  type TicketFieldKey = keyof Pick<TicketType, 'userName' | 'requestFor' | 'title' | 'content' | 'queue_val'>
  type ValidationMessages = Record<'userName' | 'requestFor' | 'title' | 'content' | 'queue_val', string>
  ...
  function querySearch(query: string, cb: (results: TicketQueueOption[]) => void) {
    ...
    cb(
      queueOptions.value.filter(
        (item) =>
          item.des.toLowerCase().includes(normalizedQuery) || item.queue.toLowerCase().includes(normalizedQuery),
      ),
    )
  }
...
<template>
  <div class="ticket-page-root">
    ...
    <el-card class="form-card">
      ...
      <el-input
        :model-value="ticket.userName"
        :placeholder="`请输入${fieldLabels.userName}`"
        clearable
        show-word-limit
        maxlength="100"
        @update:model-value="updateTicketField('userName', normalizeInputValue($event))"
      />
      <p v-if="validationMessages.userName" class="field-error">{{ validationMessages.userName }}</p>

      <el-input
        :model-value="ticket.requestFor"
        :placeholder="`请输入${fieldLabels.requestFor}（留空则使用${fieldLabels.userName}）`"
        clearable
        show-word-limit
        maxlength="100"
        @update:model-value="updateTicketField('requestFor', normalizeInputValue($event))"
      />
      <p v-if="validationMessages.requestFor" class="field-error">{{ validationMessages.requestFor }}</p>

      <el-input
        :model-value="ticket.title"
        :placeholder="`请输入${fieldLabels.title}`"
        clearable
        show-word-limit
        maxlength="100"
        @update:model-value="updateTicketField('title', normalizeInputValue($event))"
      />
      <p v-if="validationMessages.title" class="field-error">{{ validationMessages.title }}</p>

      <el-input
        :model-value="ticket.content"
        type="textarea"
        :rows="4"
        :placeholder="`请输入${fieldLabels.content}（支持换行）`"
        clearable
        show-word-limit
        maxlength="1000"
      ...
      <el-autocomplete
        :model-value="ticket.queue_val"
        :fetch-suggestions="querySearch"
        :placeholder="`请输入以筛选${fieldLabels.queue_val}`"
        value-key="queue"
        clearable
